...
const availableVideos = computed(() => videos.value?.filter(video => historyModule.isInHistory(Number(video.id))) || [])
const canShowMore = computed(() => !videoModule.isLoading && ((historyModule.getHistory?.length ?? 0) > (availableVideos.value?.length ?? 0)))
...
const showMore = function(number: number) {
    ...
    const history = historyModule.getHistory?.map(vod => Number(vod.vod_id)).slice(currentVideoIndex, currentVideoIndex + number)
...
onBeforeMount(() => {
    const history = historyModule.getHistory?.map(vod => Number(vod.vod_id)).slice(0, 20)
    if (!history?.length) return
    videoModule.fetchVideos(history).then((vids: Array<Video>|undefined) => {
        videos.value = [...(vids || [])]
    })
...
<template>
    <div class="content">
        ...
        <div v-else>
            <p v-if="!availableVideos?.length">No videos found in History.</p>
